...
class PostSearchedService(
    ...
    override fun search(keyword: String): List<PostResponse> {
        val postSearchKeyword = PostSearchKeyword.from(keyword = keyword)
        val keywordsToSearch = postSearchKeyword.keywordsToSearch()
        val postsByTitle = keywordsToSearch.map { postPort.searchByTitle(it) }.flatten()
        val postsByDescription = keywordsToSearch.map { postPort.searchByDescription(it) }.flatten()
